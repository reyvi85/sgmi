...
<div class="card">
    <div class="card-header"><h5><i class="fas fa-list"></i> Microprocesador
                ...
                <table class="table table-striped">
                    ...
                    @foreach($lista as $row)
                        <tr>
                            ...
                            <td>{{$row->nm_procesador->name}}</td>
                            <td class="text-right">
                                <form action="{{route('procesador.destroy', $row->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-plus-circle"></i> Agregar microprocesador</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{route('procesador.store', $computadora->id)}}" method="post">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="validationDefault02" placeholder="Velocidad" name="velocidad" value="{{old('velocidad')}}">
                        </div>
                            <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select" name="nm_procesador_id">
                                @foreach($tipo as $row)
                                    <option value="{{$row->id}}">{{$row->name}}</option>
                                @endforeach
                            </select>
            ...
            <div class="modal fade" id="EDIT-{{$row->id}}" tabindex="-1" role="dialog" aria-labelledby="EDIT-CD-{{$row->id}}" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{route('procesador.update', $row->id)}}" method="post">
                                @csrf
                                @method('PUT')
                                <div class="form-group">
                                    <label for="validationDefault01">Numero de Serie:</label>
                                ...
                                <div class="form-group">
                                    <label for="validationDefault02">Velocidad:</label>
                                    <input type="text" class="form-control" id="validationDefault02" placeholder="Velocidad" name="velocidad" value="{{$row->velocidad}}">
                                </div>
                                <div class="form-group">
                                    <label for="validationDefault02">Fabricante:</label>
                                    <select class="custom-select" name="nm_fabricante_id">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="custom-select" name="nm_procesador_id">
                                        @foreach($tipo as $rowC)
                                            <option value="{{$rowC->id}}" {{($rowC->id == $row->nm_procesador_id)?'selected':''}}>{{$rowC->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
