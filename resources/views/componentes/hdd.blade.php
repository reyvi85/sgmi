...
<div class="card">
    <div class="card-header"><h5><i class="fas fa-list"></i> HDD
                ...
                <table class="table table-striped">
                    ...
                    @foreach($lista as $row)
                        <tr>
                            ...
                            <td>{{$row->nm_capacidad_hdd->name}}</td>
                            <td class="text-right">
                                <form action="{{route('hdd.destroy', $row->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-plus-circle"></i> Agregar HDD</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{route('hdd.store', $computadora->id)}}" method="post">
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="validationDefault02">Capacidad del Disco:</label>
                            <select class="custom-select" name="nm_capacidad_hdd_id">
        ...
        @foreach($lista as $row)
            <div class="modal fade" id="EDIT-{{$row->id}}" tabindex="-1" role="dialog" aria-labelledby="EDIT-CD-{{$row->id}}" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-edit"></i> Editar HDD</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form action="{{route('hdd.update', $row->id)}}" method="post">
                                @csrf
                                @method('PUT')
                                <div class="form-group">
                                    <label for="validationDefault01">Numero de Serie</label>
                                ...
                                <div class="form-group">
                                    <label for="validationDefault02">Capacidad:</label>
                                    <select class="custom-select" name="nm_capacidad_hdd_id">
                                        @foreach($capacidad as $rowC)
                                            <option value="{{$rowC->id}}" {{($rowC->id == $row->nm_capacidad_hdd_id)?'selected':''}}>{{$rowC->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
