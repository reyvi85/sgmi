...
<div class="card">
    <div class="card-header"><h5><i class="fas fa-list"></i> Lector de Discos (CD/DVD)
            <button type="button" class="close" data-toggle="modal" data-target="#ADD-CD"><span aria-hidden="true" class="fas fa-plus-circle"></span> Añadir</button>
        </h5></div>
    <div class="card-body">
<!-- ########## LISTA ################ -->
        @if (count($lista) == 0)
                ...
                <table class="table table-striped">
                    ...
                    @foreach($lista as $row)
                        <tr>
                            ...
                            <td class="text-right">
                                <form action="{{route('cd.destroy', $row->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <a href="javascript:void(0);" class="btn btn-link" data-toggle="modal" data-target="#EDIT-CD-{{$row->id}}"><i class="fas fa-edit" title="Editar"></i></a>
    ...
    <div class="modal fade" id="ADD-CD" tabindex="-1" role="dialog" aria-labelledby="ADD-CD" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-plus-circle"></i> Agregar CD/DVD</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{route('cd.store', $computadora->id)}}" method="post">
                        @csrf
        ...
        @foreach($lista as $row)
            <div class="modal fade" id="EDIT-CD-{{$row->id}}" tabindex="-1" role="dialog" aria-labelledby="EDIT-CD-{{$row->id}}" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-edit"></i> Editar CD/DVD</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form action="{{route('cd.update', $row->id)}}" method="post">
                                @csrf
                                @method('PUT')
                                <div class="form-group">
                                    <label for="validationDefault01">Numero de Serie</label>
