...
<div class="card">
    <div class="card-header"><h5><i class="fas fa-list"></i> Monitor
                ...
                <table class="table table-striped">
                    ...
                    @foreach($lista as $row)
                        <tr>
                            ...
                            <td>{{$row->model}}</td>
                            <td>{{$row->nm_inch_monitor->name}}</td>
                            <td>{{$row->nm_fabricante->name}}</td>
                            <td>{{$row->nm_monitor->name}}</td>
                            <td class="text-right">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-plus-circle"></i> Agregar monitor</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{route('monitor.store', $computadora->id)}}" method="post">
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="validationDefault02">Tamaño:</label>
                            <select class="custom-select" name="nm_inch_monitor_id">
                                @foreach($inch as $row)
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select" name="nm_monitor_id">
                                @foreach($tipo as $row)
                                    <option value="{{$row->id}}">{{$row->name}}</option>
                                @endforeach
                            </select>
        ...
        @foreach($lista as $row)
            <div class="modal fade" id="EDIT-{{$row->id}}" tabindex="-1" role="dialog" aria-labelledby="EDIT-CD-{{$row->id}}" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel"><i class="fas fa-edit"></i> Editar monitor</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <form action="{{route('monitor.update', $row->id)}}" method="post">
                                @csrf
                                @method('PUT')
                                <div class="form-group">
                                    <label for="validationDefault01">Numero de Serie:</label>
                                ...
                                <div class="form-group">
                                    <label for="validationDefault02">Tamaño:</label>
                                    <select class="custom-select" name="nm_inch_monitor_id">
                                        @foreach($inch as $rowI)
                                            <option value="{{$rowI->id}}" {{($row->nm_inch_monitor_id == $rowI->id)?'selected':''}}>{{$rowI->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="custom-select" name="nm_monitor_id">
                                        @foreach($tipo as $rowC)
                                            <option value="{{$rowC->id}}" {{($rowC->id == $row->nm_monitor_id)?'selected':''}}>{{$rowC->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
